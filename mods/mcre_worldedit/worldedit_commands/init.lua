...
dofile(minetest.get_modpath("mcre_worldedit_commands") .. "/mark.lua")
dofile(minetest.get_modpath("mcre_worldedit_commands") .. "/safe.lua")
...
minetest.register_chatcommand("/cut", {
	params = "<node>",
	description = "Delete the current WorldEdit region to <node>",
	privs = {worldedit=true},
	func = safe_region(function(name, param)
		local node = get_node(name, "air")
		if not node then
			worldedit.player_notify(name, "Could not cut... sorry")
			return
		end

		local count = worldedit.set(worldedit.pos1[name], worldedit.pos2[name], node)
		worldedit.player_notify(name, count .. " nodes cut")
	end, check_region),
})
...
minetest.register_chatcommand("/green", {
	params = "",
	description = "Greens the area.",
	privs = {worldedit=true},
	func = safe_region(function(name, param)
		local newsearchnode = get_node(name, "default:dirt")
		local newreplacenode = get_node(name, "default:dirt_with_grass")
		local count = worldedit.replace(worldedit.pos1[name], worldedit.pos2[name], newsearchnode, newreplacenode)
		worldedit.player_notify(name, count .. " have green grass")
	end, check_region),
})
