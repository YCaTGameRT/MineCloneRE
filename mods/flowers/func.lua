minetest.register_entity("mcre_flowers:item",{
	hp_max = 1,
	visual="wielditem",
...
local flower_pot_remove_item = function(pos, node)
	local objs = nil
	if node and node.name == "mcre_flowers:pot" then
		objs = minetest.env:get_objects_inside_radius({x=pos.x,y=pos.y,z=pos.z}, .5)
	end
	if objs then
		for _, obj in ipairs(objs) do
			if obj and obj:get_luaentity() and obj:get_luaentity().name == "mcre_flowers:item" then
				obj:remove()
			end
		end
	end
...
flower_pot_update_item = function(pos, node)
	...
	if meta and meta:get_string("item") ~= "" then
		if node.name == "mcre_flowers:pot" then
			pos.y = pos.y
		end
		flower_tmp.nodename = node.name
		flower_tmp.texture = ItemStack(meta:get_string("item")):get_name()
		local e = minetest.env:add_entity(pos,"mcre_flowers:item")
	end
end
